<template>
    <div class="views-tushujieyue-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 图书借阅查询 </span>
                    </div>
                </template>
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="jieyuezhuangtai" label="借阅状态" width="180">
                        <template #default="{row}"> {{ row.jieyuezhuangtai }} </template>
                    </el-table-column>

                    <el-table-column label="操作" fixed="right" width="250">
                        <template #default="{row}">
                            <el-button-group>
                                <template v-if="row.jieyuezhuangtai == '借阅成功'">
                                    <el-button type="primary" :icon="Plus" size="small" @click="$router.push('/admin/tushuxujie/add?id='+row.id)">图书续借</el-button>
                                </template>

                                <template v-if="row.jieyuezhuangtai == '借阅成功'">
                                    <el-button type="primary" :icon="Plus" size="small" @click="$router.push('/admin/jieyueguihuan/add?id='+row.id)">借阅归还</el-button>
                                </template>

                                <el-tooltip effect="dark" content="详情" placement="top-start"
                                    ><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/tushujieyue/detail?id='+row.id)">详情</el-button>
                                </el-tooltip>
                            </el-button-group>
                        </template>
                    </el-table-column>
...
<script setup>
    ...
    import { canTushujieyueSelect, useTushujieyueSelect, canTushujieyueDelete } from "@/module";
    ...
    const deleteItems = (ids) => {
        return new Promise((resolve, reject) => {
            ElMessageBox.confirm("确定删除？")
                .then((res) => {
                    canTushujieyueDelete(ids).then((res) => {
                        if (res.code == 0) {
    ...
    const loadList = (page) => {
        ...
        http.post("/api/tushujieyue/selectPagesJieyueyonghu", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
