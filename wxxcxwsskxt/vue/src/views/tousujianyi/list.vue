                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column label="操作" fixed="right" width="250">
                        <template #default="{row}">
                            <el-button-group>
                                <template v-if="row.fankuizhuangtai == '待回复'">
                                    <el-tooltip effect="dark" content="编辑" placement="top-start"
                                        ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/tousujianyi/updt?id='+row.id)">反馈回复</el-button>
                                    </el-tooltip>
                                </template>
...
<script setup>
    ...
    const loadList = (page) => {
        ...
        http.post("/api/tousujianyi/selectPages", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
