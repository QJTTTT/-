<template>
    <div class="views-tushubaosun-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 图书报损查询 </span>
                    </div>
                </template>
                ...
                <div class="form-search">
                    <el-form @submit.prevent.stop :inline="true" size="small">
                        ...
                        <el-form-item label="报损编号">
                            <el-input v-model="search.baosunbianhao"></el-input>
                        </el-form-item>
                        <el-form-item>
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="baosunbianhao" label="报损编号" width="180">
                        <template #default="{row}"> {{ row.baosunbianhao }} </template>
                    </el-table-column>
                    <el-table-column prop="baosunshuliang" label="报损数量" width="180">
                        <template #default="{row}"> {{row.baosunshuliang}} </template>
                    </el-table-column>
                    <el-table-column prop="baosunshuoming" label="报损说明">
                        <template #default="{row}"> {{ row.baosunshuoming }} </template>
                    </el-table-column>
                    <el-table-column prop="jingshouren" label="经手人" width="180">
                        <template #default="{row}"> {{ row.jingshouren }} </template>
                    </el-table-column>
                    <el-table-column prop="addtime" label="报损时间">
                        ...
                        <template #default="{row}">
                            <el-button-group>
                                <el-tooltip effect="dark" content="详情" placement="top-start"
                                    ><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/tushubaosun/detail?id='+row.id)">详情</el-button>
                                </el-tooltip>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"
                                    ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/tushubaosun/updt?id='+row.id)">编辑</el-button>
                                </el-tooltip>
...
<script setup>
    ...
    import { canTushubaosunSelect, useTushubaosunSelect, canTushubaosunDelete } from "@/module";
    ...
    const deleteItems = (ids) => {
        return new Promise((resolve, reject) => {
            ElMessageBox.confirm("确定删除？")
                .then((res) => {
                    canTushubaosunDelete(ids).then((res) => {
                        if (res.code == 0) {
    ...
    const loadList = (page) => {
        ...
        http.post("/api/tushubaosun/selectPagesJingshouren", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
