...
@Service
public class TushucaigouService {

    @Resource
    private TushucaigouMapper mapper;

    /**
     *   根据id获取一行数据
     * @param id 主键id
     */
    public ResponseData<Tushucaigou> findById(Integer id) {
        return JsonResult.success(mapper.selectById(id));
    }

    public ResponseData<List<Tushucaigou>> selectAll(Wrapper<Tushucaigou> query) {
        return JsonResult.success(mapper.selectList(query));
    }

    public ResponseData<List<Tushucaigou>> selectAll() {
        QueryWrapper<Tushucaigou> wrapper = Wrappers.query();
    ...
    public ResponseData selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Tushucaigou> wrapper = mapToWrapper(map);
    ...
    public QueryWrapper<Tushucaigou> mapToWrapper(Map<String, Object> map) {
        QueryWrapper<Tushucaigou> wrapper = Wrappers.query();
        ...
        if (!StringUtil.isNullOrEmpty(map.get("caigoubianhao"))) {
            wrapper.like("caigoubianhao", map.get("caigoubianhao"));
        }
        if (!StringUtil.isNullOrEmpty(map.get("gongyingshang"))) {
            wrapper.like("gongyingshang", map.get("gongyingshang"));
        }
     ...
     * @return 搜索到得数据
     */
    public ResponseData<List<Tushucaigou>> filter(Map<String, Object> map) {
        Class<Tushucaigou> aClass = Tushucaigou.class;
        com.jntoo.db.QueryWrapper<Tushucaigou> wrapper = DB.name(aClass);
    ...
    public ResponseData selectPages(QueryWrapper<Tushucaigou> wrapper, IPage page) {
        Map result = new HashMap();
        result.put("lists", mapper.selectPage(page, wrapper));
        return JsonResult.success(result);
    }

    public ResponseData insert(Tushucaigou entityData, Map post) {
        ...
        if (entityData.getId() != null) {
            ...
            DB.execute("UPDATE tushucaigou SET caigouxiaoji = (caigoujiage*caigoushuliang) WHERE id='" + charuid + "'");

            DB.execute("UPDATE tushuxinxi SET tushukucun = tushukucun+'" + post.get("caigoushuliang") + "' WHERE id='" + post.get("tushuxinxiid") + "'");
    ...
    public ResponseData<Object> update(Tushucaigou entityData, Map post) {
        mapper.updateById(entityData);

        Integer charuid = entityData.getId();
        DB.execute("UPDATE tushucaigou SET caigouxiaoji = (caigoujiage*caigoushuliang) WHERE id='" + post.get("id") + "'");
