...
public class TushuxinxiService {
    ...
    public ResponseData webDetail(Integer id) {
        Map post = DB.name("tushuxinxi").find(id);
        Tushuxinxi map = BeanUtil.mapToBean(post, Tushuxinxi.class, true);

        DB.execute(
            "INSERT INTO liulanjilu(tushuxinxiid, tushuhao, tushumingcheng, tushuleibie, liulanyonghu) SELECT id, tushuhao, tushumingcheng, tushuleibie, '" +
            SessionFactory.getUsername() +
            "' FROM tushuxinxi WHERE id='" +
            post.get("id") +
            "' AND '" +
            SessionFactory.getUsername() +
            "'!='null'"
