...
@Service
public class TousujianyiService {

    @Resource
    private TousujianyiMapper mapper;

    /**
     *   根据id获取一行数据
     * @param id 主键id
     */
    public ResponseData<Tousujianyi> findById(Integer id) {
        return JsonResult.success(mapper.selectById(id));
    }

    public ResponseData<List<Tousujianyi>> selectAll(Wrapper<Tousujianyi> query) {
        return JsonResult.success(mapper.selectList(query));
    }

    public ResponseData<List<Tousujianyi>> selectAll() {
        QueryWrapper<Tousujianyi> wrapper = Wrappers.query();
    ...
    public ResponseData selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Tousujianyi> wrapper = mapToWrapper(map);
    ...
    public QueryWrapper<Tousujianyi> mapToWrapper(Map<String, Object> map) {
        QueryWrapper<Tousujianyi> wrapper = Wrappers.query();

        String where = " 1=1 ";
        // 以下是判断搜索框中是否有输入内容，判断是否前台是否有填写相关条件，符合则写入sql搜索语句
        if (!StringUtil.isNullOrEmpty(map.get("fankuileixing"))) {
            wrapper.eq("fankuileixing", map.get("fankuileixing"));
        }
     ...
     * @return 搜索到得数据
     */
    public ResponseData<List<Tousujianyi>> filter(Map<String, Object> map) {
        Class<Tousujianyi> aClass = Tousujianyi.class;
        com.jntoo.db.QueryWrapper<Tousujianyi> wrapper = DB.name(aClass);
    ...
    public ResponseData selectPages(QueryWrapper<Tousujianyi> wrapper, IPage page) {
        Map result = new HashMap();
        result.put("lists", mapper.selectPage(page, wrapper));
        return JsonResult.success(result);
    }

    public ResponseData insert(Tousujianyi entityData, Map post) {
    ...
    public ResponseData<Object> update(Tousujianyi entityData, Map post) {
        mapper.updateById(entityData);

        Integer charuid = entityData.getId();
        DB.execute("UPDATE tousujianyi SET fankuizhuangtai = '已回复' WHERE id='" + charuid + "'");
