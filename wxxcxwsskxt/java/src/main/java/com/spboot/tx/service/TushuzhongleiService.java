...
@Service
public class TushuzhongleiService {

    @Resource
    private TushuzhongleiMapper mapper;

    /**
     *   根据id获取一行数据
     * @param id 主键id
     */
    public ResponseData<Tushuzhonglei> findById(Integer id) {
        return JsonResult.success(mapper.selectById(id));
    }

    public ResponseData<List<Tushuzhonglei>> selectAll(Wrapper<Tushuzhonglei> query) {
        return JsonResult.success(mapper.selectList(query));
    }

    public ResponseData<List<Tushuzhonglei>> selectAll() {
        QueryWrapper<Tushuzhonglei> wrapper = Wrappers.query();
    ...
    public ResponseData selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Tushuzhonglei> wrapper = mapToWrapper(map);
    ...
    public QueryWrapper<Tushuzhonglei> mapToWrapper(Map<String, Object> map) {
        QueryWrapper<Tushuzhonglei> wrapper = Wrappers.query();

        String where = " 1=1 ";
        // 以下是判断搜索框中是否有输入内容，判断是否前台是否有填写相关条件，符合则写入sql搜索语句
        if (!StringUtil.isNullOrEmpty(map.get("fenleimingcheng"))) {
            wrapper.like("fenleimingcheng", map.get("fenleimingcheng"));
        }
     ...
     * @return 搜索到得数据
     */
    public ResponseData<List<Tushuzhonglei>> filter(Map<String, Object> map) {
        Class<Tushuzhonglei> aClass = Tushuzhonglei.class;
        com.jntoo.db.QueryWrapper<Tushuzhonglei> wrapper = DB.name(aClass);
    ...
    public ResponseData selectPages(QueryWrapper<Tushuzhonglei> wrapper, IPage page) {
        Map result = new HashMap();
        result.put("lists", mapper.selectPage(page, wrapper));
        return JsonResult.success(result);
    }

    public ResponseData insert(Tushuzhonglei entityData, Map post) {
    ...
    public ResponseData<Object> update(Tushuzhonglei entityData, Map post) {
